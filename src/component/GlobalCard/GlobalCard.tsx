...
function GlobalCard({globalData}) {
    ...
    return (
      ...
      <View>
        <Text style={[styles.globalDataTitle]}> {`Total Confirmed: ${globalData.TotalConfirmed}`}</Text>
        <Text style={[styles.globalDataTitle]}> {`Total Death: ${globalData.TotalDeaths}`}</Text>
        <Text style={[styles.globalDataTitle]}> {`Total Recovered: ${globalData.TotalRecovered}`}</Text>
        <View style={[styles.graphView,{marginTop:20}]}>
      { globalData ? <Graph data={formGlobalGraphData()} graphType = {'Line'} height={200}/> : null}
        </View>
      </View>
    </View>
    )
